// Persistent claude bridge: announces a `cc` namespace to jot, watches new
// `cc/*` channels, and runs ONE persistent `claude -p` per channel using
// stream-json so the session is preserved across user messages.
//
// Output (assistant messages, session-start, etc.) lands in the channel via
// the existing jot hooks — bridge does not parse stdout to avoid duplicates.
...
export async function runBridge(extraArgs: string[]) {
  ...
  function sendUserMessage(channel: string, text: string) {
  ...
  async function watchChannel(channel: string) {
    ...
    while (true) {
      ...
      since = fragment.id;
      sendUserMessage(channel, fragment.markdown);
    }
  }
